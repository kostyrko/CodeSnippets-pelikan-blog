...
Wtyczka sassowa może przyjąć dodatkowe argumenty takie jak **outputStyle** (zdefiniowanie stylu w jakim zostanie zapisany CSS -> expanded/nested/compact/compressed) czy **sourceComments** (mapuje scss w css przy pomocy wtyczki **gulp-sourcemaps**, pozwala na łatwiejsze odnalezienie deklaracji w pliku scss), zapisane w formie obiektu
...
Przykładowo
        ...
        return gulp.src('./sass/main.scss') 
            .pipe(sourcemaps.init())            // inicjalizacja wtyczki gulp-sourcemaps
            .pipe(sass({
                outputStyle: 'expanded',
                sourceComments: 'map'            // komentarze map(gulp-sourcemaps)
            }).on("error", sass.logError))      // w przypadku błędu wypisz w konsoli
            .pipe(sourcemaps.write())           // zmapuj przy pomocy gulp-sourcemaps
            .pipe(gulp.dest('css'))  
        });
